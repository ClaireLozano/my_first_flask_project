...
def splitByWord(line):
	"""
    Get a list of words from string

    Parameters
    ----------
    line : str
        A string to split into a word list

    Returns
    -------
    []
        ['pueden', 'consultar', 'en', 'el', 'sitio', 'de', 'Internet', 'del', 'GEE:']
...
def getDataFromText(text, precision):
	...
	with open(jsonFile, 'r', encoding="utf8") as f:
		...
		listSortedWords = sortByWord(text, int(precision))

		# Find the language of a text
		return [findLanguage(listSortedWords, data, int(precision)), data]